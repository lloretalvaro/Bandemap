...
final class RemoteDataSourceTests: XCTestCase {
    ...
    func testRemoteDataSource_whenGetGeocodingInfoResultsIsEmpty_expectGeocodingInfoResultsEmpty() async throws{
        // Le metemos el request handler con status code y la data que queramos
        URLProtocolMock.requestHandler = { request in
            let response = HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!
            let geocodingInfo = GeocodingInfo(documentation: nil, licenses: nil, rate: nil, results: [], status: StatusInfo(code: 200, message: "OK"), stayInformed: nil, thanks: nil, timestamp: nil, totalResults: 0)
